...
#           Use WUP similarity algorithm here
    ...
    result = (max_score,)
    if EXPLAIN: result += (f'"{w1}" -> "'+ best_matching_word['x']+ f'" < match with > "{w2}" -> "'+ best_matching_word['y']+ f'" | matching score = '+ str(best_matching_word['score']),)
    return result
...
def check_2_sentence_similarities(X, Y, USE_WORDNET_SIMILARITY = True):
    ...
    if EXPLAIN: print(f' axis names of the combined words\'s vector space: {rvector}')
    if USE_WORDNET_SIMILARITY: 
        for w in rvector:
            X_set_match = max([word_similiarity(w,X_set_word) for X_set_word in X_set])
            X_set_scores = X_set_match[0]
            if EXPLAIN: print(f' → word in event: "{w}", analysis | ', X_set_match[1])
            if X_set_scores: l1.append(X_set_scores) # create a vector
            else: l1.append(0)
                
            Y_set_match = max([word_similiarity(w,Y_set_word) for Y_set_word in Y_set])
            Y_set_scores = Y_set_match[0]
            if EXPLAIN: print(f' → word in scenario: "{w}", analysis | ', Y_set_match[1])
            if Y_set_scores: l2.append(Y_set_scores)
            else: l2.append(0)
    else:
        for w in rvector:
            if w in X_set: l1.append(1) # create a vector
            else: l1.append(0)
            if EXPLAIN: print(f' → word in event: "{w}", analysis | ', l1[-1])
            if w in Y_set: l2.append(1)
            else: l2.append(0)
            if EXPLAIN: print(f' → word in scenario: "{w}", analysis | ', l2[-1])
#     
    if EXPLAIN: print(f' event {X_set} makes vector: {l1} and scenario {Y_set} makes vector: {l2}')
    c = 0
    ...
    print(f' similarity between event "{X}" and scenario "{Y}": ', '\033[1m' + str(cosine) + '\033[0m')
    return cosine
...
def find_principles(event, sp_df):
    sorted_sp_paris = sorted([(check_2_sentence_similarities(event, row['scenario']), row['principle']) for idx, row in sp_df.iterrows()])[::-1]
    return [pair[1] for pair in sorted_sp_paris if pair[0]][:5]
...
class Users(Resource):
    ...
    def post(self):
        ...
        event = args['event']
        # print(f'*** most recommended principles found for event \"{event}\" are {find_principles(event, sp_pairs)} ***')
        return {'data': f'most recommended principles found for event \"{event}\" are {find_principles(event, sp_df)} '}, 200  # return data with 200 OK
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=8078)  # run our Flask app
