...
class UpNlpClient:
    ...
    def word_similiarity(self, w1, w2):
        ...
        max_score = 0
        if EXPLAIN:
            best_matching_word = {"x": "", "y": "", "score": 0}
        for x in sl1:
            for y in sl2:
        ...
        if EXPLAIN:
            result += (
                f'"{w1}" -> "'
                + best_matching_word["x"]
                + f'" < match with > "{w2}" -> "'
                + best_matching_word["y"]
                + f'" | matching score = '
                + str(best_matching_word["score"]),
            )
        return result
    ...
    def check_2_sentence_similarities(self, X, Y, USE_WORDNET_SIMILARITY=True):
        ...
        if USE_WORDNET_SIMILARITY:
            for w in rvector:
                X_set_match = max(
                    [self.word_similiarity(w, X_set_word) for X_set_word in X_set]
                )
                ...
                Y_set_match = max(
                    [self.word_similiarity(w, Y_set_word) for Y_set_word in Y_set]
                )
                Y_set_scores = Y_set_match[0]
                if EXPLAIN:
                    print(f' → word in scenario: "{w}", analysis | ', Y_set_match[1])
                if Y_set_scores:
        ...
        if EXPLAIN:
            print(
                f' similarity between event "{X}" and scenario "{Y}": ',
    ...
    def find_principles(self, event):
        ...
        sorted_sp_paris = sorted(
            [
                (
                    self.check_2_sentence_similarities(event, row["scenario"]),
                    row["principle"],
                )
                for idx, row in self.sp_df.iterrows()
            ]
        )[::-1]
    ...
    def gpt3_find_matching_scenarios(self, event, scenarios_df):
        ...
        for i, row in top_senarios_df.iterrows():
            # TODO: convert the event_tags and row['tags'] to set will speed this up
            score = self.check_2_sentence_similarities(
                row["scenarios"], event["event_title"]
            )
        ...
        top_senarios_df = top_senarios_df.sort_values(
            ["scentence_similarity"], ascending=[False]
        ).head(number_senarios_for_getting_principles)
    ...
    def format_principles(self, principles_df, principle_scenario_pairs_df):
        ...
        for i, row in principles_df.iterrows():
            scenario = principle_scenario_pairs_df.loc[
                principle_scenario_pairs_df["principles"] == row["id"], "scenarios"
            ].tolist()[0]
