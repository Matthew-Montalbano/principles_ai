...
def notionPropertiesToObject(p):
    return {
        ...
        "principles": p["properties"]["Principles Handpicked"]["rich_text"][0][
            "plain_text"
        ],
    }
...
class NotionClient:
    ...
    def get_user_info(self, calendar_id):
        data = {"filter": {"property": "calendarId", "text": {"equals": calendar_id}}}
        res = r.post(
            "https://api.notion.com/v1/databases/f14457c2d77b488d8d379c637cce6e76/query",
    ...
    def create_page(self, event):
        ...
        new_page = {
            ...
            "properties": {
                "Event ID": {"title": [{"text": {"content": event.get("event_id")}}]},
                "User Email": {"email": event.get("user_email")},
                "User Name": {
                    "rich_text": [{"text": {"content": event.get("user_name")}}]
                },
                "Event Name": {
                    "rich_text": [{"text": {"content": event.get("event_name")}}]
                },
                "calendarId": {
                    "rich_text": [{"text": {"content": event.get("calendar_id")}}]
                },
                "Event Description": {
                    "rich_text": [{"text": {"content": event.get("event_description")}}]
                },
                "Location": {
                ...
                "openAiPredictions": {
                    "rich_text": [
                        {"text": {"content": event.get("openai_predictions")}}
                    ]
                },
        ...
        data = {
            "filter": {
                "and": [
                    ...
                    {"property": "calendarId", "text": {"equals": calendar_id}},
                ]
            }
        }

        res = r.post(
            f"https://api.notion.com/v1/databases/{self.notion_db}/query",
            data=json.dumps(data),
            headers=self.headers,
        )
    ...
    def get_scenarios_table(self):
        res = r.post(
            f"https://api.notion.com/v1/databases/{NOTION_SCENARIOS}/query",
            headers=self.headers,
        )
        results = json.loads(res.text)["results"]
        ...
        for result in results:
            ...
            new_obj["scenarios_human"].append(
                props.get("Scenario").get("title")[0].get("plain_text")
            )
            ...
            new_obj["tags"].append(
                list(
                    map(lambda x: x.get("name"), props.get("Tags").get("multi_select"))
                )
            )
            new_obj["principles"].append(
                list(
                    map(lambda x: x.get("id"), props.get("Principles").get("relation"))
                )
            )
    ...
    def get_principles_from_list(self, principles):
        ...
        new_obj = {"principle": [], "source": [], "notes": [], "id": []}

        for principle in principles:
            props = json.loads(
                r.get(
                    f"https://api.notion.com/v1/pages/{principle}", headers=self.headers
                ).text
            ).get("properties")
            new_obj["id"].append(principle)
            new_obj["principle"].append(
                props.get("Principle (Work/Life Principles, Best Practices)")
                .get("title")[0]
                .get("plain_text")
            )
            if len(props.get("Notes (detailed action items)").get("rich_text")) > 0:
                new_obj["notes"].append(
                    props.get("Notes (detailed action items)")
                    .get("rich_text")[0]
                    .get("plain_text")
                )
            else:
                ...
                for source in props.get("Sources").get("relation"):
                    ...
                    if source_id in source_cache:
                        ...
                        new_source = (
                            json.loads(
                                r.get(
                                    f"https://api.notion.com/v1/pages/{source_id}",
                                    headers=self.headers,
                                ).text
                            )
                            .get("properties")
